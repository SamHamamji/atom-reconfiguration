...
static bool *get_exclusion_array_parallel(const struct Interval *interval,
                                          const int *height_array) {
  ...
  for (int i = 0; i < MAX_THREADS_NUMBER; i++) {
    ...
    for (int height = inputs[i].height_range.min_height;
      ...
      exclusion_array[threads_output[height - 1]] = true;
    }
  }
...
parallel_solver_test_function(const struct Interval *const interval) {
  ...
  bool *exclusion_array = get_exclusion_array_parallel(interval, height_array);
